...
train_dataset = datasets.MNIST(root='/Users/max/WASP_AIandML_Assignment/nesy_semanticloss/code/data', train=True,  download=True, transform=transform)
test_dataset  = datasets.MNIST(root='/Users/max/WASP_AIandML_Assignment/nesy_semanticloss/code/data', train=False, download=True, transform=transform)
train_loader = DataLoader(train_dataset, batch_size=64, shuffle=True)
test_loader  = DataLoader(test_dataset,  batch_size=1000, shuffle=False)
# Model Definition
class MNISTSigmod(nn.Module):
    def __init__(self):
        super().__init__()
        self.net = nn.Sequential(
            ...
            nn.Sigmoid()           # chage to sigmoid activation function
        )
...
model = MNISTSigmod()
#critierion = nn.NLLLoss()  # for Q2.1 keep the NLLLoss since we are using Sigmoid activation, which outputs probabilities. We will take log of the output before passing to NLLLoss.
critierion = nn.BCELoss()  # for Q2.2 use BCELoss since we are treating this as a multi-label classification problem with sigmoid activation.
...
# for Q2.2 we need to convert the labels to one-hot encoding since we are using BCELoss which expects the target to be of the same shape as the output.
def to_onehot(labels, num_classes=10):
    return torch.zeros(labels.size(0), num_classes).scatter_(1, labels.unsqueeze(1), 1.0)
...
def train_one_epoch(model, loader, optimizer, criterion, start_iteration=0):
    ...
    for images, labels in loader:
        optimizer.zero_grad()
        probs = model(images)
        targets = to_onehot(labels) # Convert labels to one-hot encoding for BCELoss
...
def evaluate(model, loader, criterion):
    ...
    with torch.no_grad():
        for images, labels in loader:
            probs = model(images)
            targets = to_onehot(labels)          # ← convert to one-hot
            loss    = criterion(probs, targets) 
...
with open('/Users/max/WASP_AIandML_Assignment/nesy_semanticloss/code/Q2_history/Q22model_epoch_history.json', 'w') as f:
    json.dump(model_epoch_history, f)
print("Model epoch history saved to 'model_epoch_history.json'.")

with open('/Users/max/WASP_AIandML_Assignment/nesy_semanticloss/code/Q2_history/Q22model_iteration_history.json', 'w') as f:
